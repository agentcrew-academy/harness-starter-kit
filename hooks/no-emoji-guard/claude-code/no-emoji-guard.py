...
# stdout and stderr take the locale codec too. Where that is not UTF-8, this
# hook blocks correctly and then dies with UnicodeEncodeError while printing
# its own message, so the user sees a traceback instead of the reason.
...
def read_payload():
    ...
    Reading bytes removes the dependency on the ambient locale entirely.
    """
